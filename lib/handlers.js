  ...
  if (data.method === 'get') {
    ...
    if (trimmedAssetName.length > 0) {
      ...
      helpers.getStaticAsset(trimmedAssetName, (err, data) => {
        if (!err && data) {
          // Determine the content type (daefault to plain text)
                    ...
                    _data.delete('checks', id, (err) => {
                      ...
                      if (checksDeleted === checksToDelete) {
                        if (!deletionErrors) {
                          ...
                          callback(500, {
                            Error:
                              'errors encountered whiler attempting to delete all of the users checks. All checks may not have been deleted from the system successfully.',
                          })
                        }
                      }
                    })
...
// Checks - post
// Required data: protocol, successCodes, method, successCodes, timeoutSeconds
// Optional data: none
        ...
        _data.read('users', userPhone, (err, userData) => {
          if (!err && userData) {
            ...
            if (userChecks.length < config.maxChecks) {
              ...
              // Create the check object, and include the user's name
              let checkObject = {
                id: checkId,
                userPhone: userPhone,
                protocol: protocol,
              ...
              _data.create('checks', checkId, checkObject, (err) => {
                if (!err) {
                  // Add the data to the user's object
                  ...
                  callback(500, {
                    Error: 'could not create the new check object',
                  })
                }
              })
            } else {
              callback(400, {
                Error: `the user already hit max number of checks ${config.maxChecks}`,
              })
            }
          } else {
            callback(403)
  ...
  if (id) {
    ...
    _data.read('checks', id, (err, checkData) => {
      if (!err && checkData) {
                  ...
                  _data.read('users', checkData.userPhone, (err, userData) => {
                    if (!err && userData) {
                      ...
                      if (checkPosition > -1) {
                        ...
                        callback(500, {
                          Error:
                            'could not find the check on the user object, couldnt remove',
                        })
                      }
                    } else {
                      callback(500, {
                        Error:
                          'could not find user who created the check, so could not remove the check',
                      })
                    }
                  })
                } else {
        ...
        callback(400, { Error: 'The specified id doesnt exist' })
      }
    })
  } else {
    callback(400, { Error: 'Missing required field' })
